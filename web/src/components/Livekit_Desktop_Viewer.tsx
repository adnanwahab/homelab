...
interface Window {
 ...
 function DesktopViewer() {
  const [windows, setWindows] = useState<Window[]>([
    ...
    { id: 3, title: "Window 3", content: "Content for window 3", isMaximized: false },
    { id: 4, title: "Window 4", content: "Content for window 4", isMaximized: false },
    { id: 5, title: "Window 5", content: "Content for window 5", isMaximized: false },
  ])
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      <div className={`grid gap-4 ${windows.some(w => w.isMaximized) ? '' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5'}`}>
        {windows.map((window) => (
          <Card 
            key={window.id}
          ...
          >
            <CardHeader className="border-b p-4">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-medium">{window.title}</CardTitle>
