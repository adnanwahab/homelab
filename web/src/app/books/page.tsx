...
export default function BooksTablePage() {
  ...
  useEffect(() => {
    // Transform the list into an array of objects with a rank and title
    const data = twenty_twenty_four.map((title, i) => ({
      rank: i + 1,
      title,
    }));

    // Create the table using Observable Plot's table function.
    // (Check the Plot documentation for additional customization options.)
    const table = Plot.table(data, {
      columns: ["rank", "title"],
      // Optional: adjust cell padding, borders, etc.
    });
