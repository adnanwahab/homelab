...
// const JOLT_PATH =
//   "https://cdn.jsdelivr.net/npm/jolt-physics@0.29.0/dist/jolt-physics.wasm-compat.js";

const JOLT_PATH = `js/jolt-physics.wasm-compat.js`;
...
async function JoltPhysics() {
  ...
  function createInstancedBody(mesh, mass, restitution, shape) {
    ...
    for (let i = 0; i < mesh.count; i++) {
      ...
      const quaternion = _quaternion.setFromRotationMatrix(
        _matrix.fromArray(array, i * 16),
      ); // TODO Copilot did this
    ...
    for (let i = 0, l = meshes.length; i < l; i++) {
      ...
      if (mesh.isInstancedMesh) {
        ...
        for (let j = 0; j < bodies.length; j++) {
          ...
          _matrix
            .compose(_position, _quaternion, _scale)
            .toArray(array, j * 16);
        }
