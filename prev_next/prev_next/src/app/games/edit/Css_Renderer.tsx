...
import styles from './CssRenderer.module.css';
...
export default function CssRenderer({ className }: CssRendererProps) {
  ...
  const transform = (targets: THREE.Object3D[], duration: number) => {
    ...
    new TWEEN.Tween({})
      .to({}, duration * 2)
      .onUpdate(() => rendererRef.current?.render(sceneRef.current!, cameraRef.current!))
      .start();
  };
  ...
  const handleLayoutChange = (layout: keyof typeof targetsRef.current) => {
  ...
  return (
    <div className={className}>
      ...
      <div className={styles.menu}>
        <button onClick={() => handleLayoutChange('table')}>TABLE</button>
        <button onClick={() => handleLayoutChange('sphere')}>SPHERE</button>
        <button onClick={() => handleLayoutChange('helix')}>HELIX</button>
