git import { OpenAI } from "openai";
import pdfParse from "pdf-parse";

const papersDir = "/home/adnan/derp/support_bret/papers";
...
async function processChunk(pdfFile) {
    console.log(pdfFile);
    //const pdfText = await pdfParse(pdfFile).then(data => data.text);
    //const pdfText = "hello"

    //return pdfText
    //  by passing the base64 encoded image directly in the request. Images can be passed in the user messages.
...
async function main() {
    ...
    fs.readdirSync(papersDir).forEach((file) => {
        const processed = processChunk(file)
            .then
            //fs.writeFileSync(`diagrams/${file}.md`, processed)
            ();
    });
}
...
function assign_roles() {
    ...
    const module_buckets = {
        ...
        Prediction: [],
        Simulation: [],
        RealWorldApplications_Dynamical_Systems_Theory: [],
        MechanicalEngineering_ElectricalEngineering_HardwareRepairMaintence: [],
    };
    ...
    const _ = [
        ...
        },
        {
            ...
            pages: [
                ...
                {
                    name: "Motion Trajectory Prediction",
                    path: "motion-prediction",
                },
                {
                    name: "LLMs vs Classical Planning",
                    path: "llms-vs-classical",
                },
            ],
        },
        {
        ...
        },
        {
            ...
            pages: [
                ...
                {
                    name: "Electrical Engineering Essentials + Embedded Systems",
                },
                {
                    name: "Kinematics and Dynamics",
                    path: "kinematics-dynamics",
                },
                { name: "Essential Robotics Tools", path: "robotics-tools" },
                { name: "Fault Diagnosis", path: "fault-diagnosis" },
                {
                    name: "Motor and Sensor Repair",
                    path: "motor-sensor-repair",
                },
                {
